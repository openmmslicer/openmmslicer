...
from openmmslicer.fe_estimators import ParallelNumbaFunction as _ParallelNumbaFunction, EnsembleMBAR as _EnsembleMBAR


def _expected_transition_matrix(energy, free_energy, log_weights):
    """A numba function which calculates the transition matrix based on energies, free energies and log weights."""
...
class ExpectedRoundTripTime:
    """
    Calculates the transition matrix, the mean first-passage times and the round-trip time associated with a particular
    ...
    @staticmethod
    def _cast(lambdas):
        """numpy.ndarray : Cleans up an input sequence of lambda values."""
        if len(lambdas) < 2:
    ...
    def expectedTransitionMatrix(self, lambdas):
        """
        The expected transition matrix associated with a sequence of lambda values.
    ...
    def expectedTransitionTime(self, lambdas, lambda0=0., lambda1=1., target0=1, target1=0, costs=None,
        ...
        """
        The mean first-passage time between two directed lambda values.
        ...
        lambda0 : float, optional
            The initial lambda value. Default is 0.
        lambda1 : float, optional
            The final lambda value. Default is 1.
        target0 : float, optional
            The target lambda value corresponding to the initial lambda value. Default is 1.
        target1 : float, optional
            The target lambda value corresponding to the final lambda value. Default is 0.
        costs : list or np.ndarray, optional
            The associated costs for each of the lambda values. Default is equal costs.
        transition_matrix : np.ndarray, optional
            If supplied, this is the transition matrix which will be used to estimate the expected transition time.
    ...
    def expectedRoundTripTime(self, *args, lambda0=0., lambda1=1., **kwargs):
        """
        The round-trip time between two lambda values.
        ...
        lambda1 : float, optional
            The final lambda value. Default is 1.
        args
            Positional arguments to be passed to expectedTransitionTime().
        kwargs
            Keyword arguments to be passed to expectedTransitionTime().
