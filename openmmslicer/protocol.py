...
import openmmslicer.misc as _misc
...
class Protocol:
    """
    An object which represents the lambda values of an alchemical protocol.

    Parameters
    ----------
    protocol : list or numpy.ndarray
        The sequence of lambda values defining the protocol.
    kwargs
        Keyword arguments initialising the rest of the attributes.

    Attributes
    ----------
    value : numpy.ndarray
        The sequence of lambda values defining the protocol.
    fixed_values : list or numpy.ndarray
        If not None, these values will always be enforced to be part of the protocol.
    significant_figures : int
        If not None, he protocol will be rounded to this number of significant figures.
    round_fixed_values : bool
        If True and significant_figures is not None, the fixed_values will be rounded as well.
    """
    def __init__(self, protocol, fixed_values=None, significant_figures=None, round_fixed_values=False):
        self.significant_figures = significant_figures
        self.fixed_values = fixed_values
        self.round_fixed_values = round_fixed_values
        self.value = protocol

    @property
    def fixed_values(self):
        if self.round_fixed_values:
            return self._round(self._fixed_values)
        else:
    ...
    def fixed_values(self, val):
        if val is None or not len(val):
            val = []
        self._fixed_values = _np.asarray(val)
    ...
    def value(self, val):
        if val is None or not len(val):
            val = []
        val = sorted(set(val) - set(self.fixed_values))
        self._value = _np.sort([*self._round(val), *self.fixed_values])
...
class OptimisableProtocol(Protocol):
    """
    A dynamic protocol which can be optimised on the fly.

    Parameters
    ----------
    ensemble : object
        A sampler from openmmslicer.samplers.
    update_func : callable, optional
        A callable which when called as update_func(self) returns the number of times self.ensemble.sample() will be
        called before the next protocol update is performed. Default is
        100 + 0.1 * self.ensemble.fe_estimator.effective_sample_size.
    args
        Positional arguments passed to super().__init__().
    kwargs
    ...
    ----------
    ensemble : object
        The supplied sampler from openmmslicer.samplers.
    update_func : callable
        The protocol update function.
    effective_lambda_bounds : tuple
        A tuple of the minimum and maximum lambda values visited since the last optimisation.
    """
    ...
    def fixed_values(self):
        return _np.asarray(sorted(set(Protocol.fixed_values.fget(self)) | {self.ensemble.lambda_}))
    ...
    def _initial_protocol_guess(self, n, fixed_values=None):
        if fixed_values is None:
            fixed_values = self.fixed_values
        if n in self._protocol_memo:
            protocol = self._protocol_memo[n]
        elif any(k for k in self._protocol_memo if k > n):
            protocol = self._protocol_memo[sorted(k for k in self._protocol_memo if k > n)[0]]
        elif any(k for k in self._protocol_memo if k < n):
            protocol = self._protocol_memo[sorted(k for k in self._protocol_memo if k < n)[-1]]
        else:
    ...
    def effective_lambda_bounds(self):
        lambdas_recent = self.ensemble.fe_estimator.walker_memo.timestep_lambdas[self._prev_optimisation:]
    ...
    def optimiseContinuous(self, n_opt, fixed_values=None, start=0., end=1., tol=1e-3, **kwargs):
        """
        Performs a continuous optimisation of the current protocol.

        Parameters
        ----------
        n_opt : int
            The number of optimisable values in addition to the fixed ones.
        fixed_values : list or np.ndarray, optional
            The fixed values which will be enforced in the protocol.
        start : float, optional
            This is the value below which no optimisation will be performed. Default is 0.
        end : float, optional
            This is the value above which no optimisation will be performed. Default is 1.
        tol : float, optional
            The convergence tolerance. Default is 1e-3.
        kwargs
            Keyword arguments to be passed to cma.fmin() options parameter.
        ...
        fun : float
            The value of the optimised function.
        success : bool
    ...
    def optimiseDiscrete(self, fixed_values=None, start=0., end=1., **kwargs):
        """
        Performs a brute-force discrete optimisation of the length current protocol by calling optimiseContinuous() on
        a range of protocols of different lengths.

        Parameters
        ----------
        kwargs
            Keyword arguments to be passed to optimiseContinuous().
    ...
    def optimise(self, start=0., end=1., **kwargs):
        """
        Optimises the protocol using optimiseDiscrete() in a more failsafe way.

        Parameters
        ----------
        kwargs
            Keyword arguments to be passed to optimiseDiscrete().
        """
        ...
        if not _np.isinf(fun) and not _np.isnan(fun) and success:
            self.value = protocol
            tau = fun * self.ensemble.simulation.integrator.getStepSize().in_units_of(_unit.nanosecond)
